...
class Jogo(models.Model):
    ...
    desenvolvedora = models.ForeignKey(
        Desenvolvedora, on_delete=models.PROTECT, related_name="jogos"
    )
    motorgrafico = models.ForeignKey(
        MotorGrafico, on_delete=models.PROTECT, related_name="jogos"
    )
...
class Assinatura(models.Model):
    ...
    usuario = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="Assinaturas"
    )
    status = models.IntegerField(
        choices=StatusAssinatura.choices, default=StatusAssinatura.NAOASSINANTE
    )

    @property
    def total(self):
        queryset = self.itens.all().aggregate(
            total=models.Sum(F("quantidade") * F("PacoteAssinatura__preco"))
        )
...
class ItensAssinatura(models.Model):
    Assinatura = models.ForeignKey(
        Assinatura, on_delete=models.CASCADE, related_name="itens"
    )
    PacoteAssinatura = models.ForeignKey(
        PacoteAssinatura, on_delete=models.PROTECT, related_name="+"
    )
